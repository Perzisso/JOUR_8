...
puts  "Choisissez parmi les 8 options suivantes en tapant le numéro associé :
...
(nombre de handle avec 1 caractère, nombre de handle avec 2 caractères,etc)"
puts "> "

response = gets.chomp
menu_choice = ["1", "2", "3", "4", "5", "6", "7", "8"]
...
if response == "2"
    ...
    381.times do |i|
      length = journalists[i].tr("@", "").length
      size << length
    end
    journalists_size = size.zip(journalists).to_h
    journalistes_size_sort = journalists_size.sort
    size_sort = size.sort
    
    puts "le nom d'utilisateur twitter le plus court est : #{journalists[161].tr("@", "")} avec #{size.sort[0]} caractères"
    puts " - "*12
end

if response == "3"
    count = journalists.count { |hdle| hdle.length - 1 == 5 }
    puts "Il y a #{count} handle comptant 5 caracères"
    puts " - "*12
end

if response == "4"
    count = journalists.count { |hdle| hdle[1].match(/[A-Z]/) }
    puts "Il y a #{count} handel qui commencent par une majuscule"
    puts " - "*12
end
...
if response == "6"
    puts journalistes_size_sort
    puts " - "*12
end

if response == "7"
    puts "L'utilisateur @epenser est à la #{journalists.index("@epenser")}ème position dans l'array"
    puts " - "*12
end

if response == "8"
    journalists_sort = journalists.first(381).sort_by { |hdle| hdle.length }
    hdle_lengths = journalists_sort.group_by {|hdle| hdle.length}
        hdle_lengths.each do |length, hdle|
        puts "Il y a #{hdle.size} handel de #{length - 1} caractères"
        puts " - "*12
        end
end
